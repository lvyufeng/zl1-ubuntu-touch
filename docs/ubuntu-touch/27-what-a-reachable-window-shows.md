...
三条更正有个共同形状：**从一个局部读数推广成了全局结论。**

| 读数 | 我当时的结论 | 实际 |
| --- | --- | --- |
| `carrier=1`、`operstate=up` | 链路是通的 | 这两个字段不反映包能不能过去（6 月就栽在这，这次仍然成立） |
| 见过几次卡死 | "约 1/4 次开机会卡死" | 是"每次开机头 ~50 秒通"这个时刻表 |
| 兜底代码写好了 | 到点会自己回 recovery | 循环里的 `continue` 把它整段跳过，从未执行过 |

对应的做法很简单：**能测的就去测**。这一轮每一处更正都来自一次实测——
10 分钟的可达性采样、一次状态页抓取——没有一处来自读代码。
`measure-link-stability.sh` 存在的意义就是把"链路好不好用"变成数字，
而不是留给我下一个判断。
