...
CALLEE_NAMES='EDLPM|BOOTADDR|KEEPER|HEALTH|MODEM|SLEEP|LMH|LEDS|VIBR|VIDEO|GPS|FP|ORIENT|NOEDL'
...
want '^CALLEE ORIENT$' "$(printf '%s\n' "$O" | sed -n '11p')" "then the orientation survey"
[ "$(order | wc -l)" = 11 ] && ok "exactly eleven steps -- the six read-only probes ARE among them and the 05/06 pair are NOT" \
...
[ "$(scpacts | wc -l)" = 9 ] && ok "nine pushes: the nine DEVICE steps, and no more" \
...
for f in 01-edl-postmortem 02-boot-address 03-keeper-status 04-health-check 04b-modem 04c-sleep-throttle 04d-lmh 04e-leds 04f-vibrator 04g-video 07-orientation; do
...
[ "$(find "$OD" -name '*-*.txt' | wc -l)" = 12 ] && ok "and the file set does not grow (12 archived outputs, not 24)" \
...
[ "$(order | wc -l)" = 11 ] && ok "leaving eleven steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 11 ] && ok "leaving the same eleven steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 12 ] && ok "twelve steps -- the eleven of the default plus the one that writes" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 11 ] && ok "all eleven ran (the default set; no 05/06 probe is in it)" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 10 ] && ok "all ten steps still ran" || bad "$(order | wc -l) steps ran"
