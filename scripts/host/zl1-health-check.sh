...
always "   scripts/host/zl1-post-recovery-capture-selftest.sh, 121 checks, and five mutations each"
...
always "      Offline and device-free: scripts/host/zl1-gps-selftest.sh, 99 checks (its section 9 is"
...
always "      names the rung the evidence stops at and exits 0 only for that state; it also says what it"
always "      CANNOT decide -- the lshal table's columns are not parsed on purpose (nothing in this tree"
always "      records what they mean, and a verdict on a guessed column would be fabricated)"
