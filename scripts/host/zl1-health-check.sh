...
always "      offline: scripts/host/zl1-loc-fp-selftest.sh, 84 checks. It found two real defects (doc 97):"
always "      --quiet used to silence the permission-bypass warning itself, and --create-store-dir used to"
always "      create BOTH candidate store paths while its own section 2 had decided which one biometryd"
always "      passes. Read the UNDO line against what was created -- that correspondence is the whole check)"
