...
#
# Usage: zl1-sensorfw-probe.sh [--load-all] [--settle N] [--gap N] [--keep N]
...
age_of() {
  _ts=$(printf '%s' "$1" | sed -n 's/.*uint64 \([0-9][0-9]*\).*/\1/p')
  [ -n "$_ts" ] || { printf '?'; return; }
...
report() {
  ...
  _ageb=$(age_of "$_b")
  printf '    %s.%s\n' "$_iface" "$_prop"
  printf '      read #1: %s   (last sample %s s ago)\n' "$_a" "$_agea"
  printf '      read #2: %s   (last sample %s s ago)\n' "$_b" "$_ageb"
  if [ "$_a" != "$_b" ]; then
...
echo "never received a sample from this sensor. Do NOT restart sensorfwd to 'fix' it -- on this"
echo "device a sensorfwd restart is what kills the container's sensors HAL (docs 71)."
