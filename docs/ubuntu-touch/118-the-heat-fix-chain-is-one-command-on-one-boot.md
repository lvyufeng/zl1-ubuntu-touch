...
| 离线验证？ | `zl1-heat-fix-chain-selftest.sh` **95 检查 / 0 失败**，其中 5 个变异各自改变它观察到的东西（**2026-09-24 更正：这两个数字都已过期——先是 107 检查 / 7 个变异，[`128`](128-the-chain-changed-the-heat-and-never-measured-it.md) 之后是 **159 检查 / 10 个变异**——因为这条链现在**测量**它装的那两个修复，而那个测量又带出了一节关于「这个 harness 没有改动它测的那棵树」的检查；见 `health-check` 里那条引用；本行是写这一篇时的数字，留着可见） |
...
### 4.4.1 修订（docs 120）：这道 licence 管的是第 5 步，**不管第 6 步**

这一条是本轮改掉的，而且它改的是一处**刻意的、当时还被断言着的**行为，所以说清理由。

原来的链条在证明不干净时 `exit 1`，**第 6 步（governor）也不跑**。但第 4 步证明的是"**还有谁拥有
rndis0 的地址**"——而 keeper 就是今天拥有它们的东西，**退休 keeper** 才是可能让地址变成无人拥有的动作。
governor 与这件事**没有依赖关系**：它改的是 cpufreq 的调度策略，**不移除任何进程、不碰任何地址、不重启
任何东西**（它自己的头文件里明写着它**故意不**停 keeper）。

而 docs 112 量到的是：**这个 verdict 是一场竞态，`inconclusive` 是大多数 boot 上的预期读数。**于是原来的
写法等于：**大多数 boot 上一半的发烫修复都进不去**，而其中只有一半需要那张 licence。所以现在：

* 证明不干净 → **keeper 留在原地**（拒绝不变，安全的那一边不变），
* **但 governor 照装**，输出里明写"这一半不需要那张 licence"，并在读回之后明写
  **`THE KEEPER IS STILL IN PLACE`** ——缺的是哪一半，读者不用猜。

这条与"某一步失败就停"**不是同一条规则**，差别在设备被留在什么状态：**跑过而且失败的**一步留下的状态
没人读过，停下来是对的；**拒绝**留下的是这一支刚刚描述过的那个状态（keeper 活着、地址有人拥有、手机够
得着）。
...
|---|---|
| 0 | 链条跑到头 |
| 1 | 链条没跑完：某一步失败，**或者**证明没有授权第 5 步——归档说得出是哪一种，以及 governor 那一半进没进 |
...
## 5. 离线验证：107 检查，7 个变异，以及写它时抓到的两类假绿

> **2026-09-24 追记**：下面这一节写的是本页当时的数字，**留着可见**。这条 harness 现在是
> **159 检查 / 10 个变异**：链接上了 `device/zl1-thermal.sh --ab` 去测自己装的两个修复，
> 而写那段夹具时查出这个 harness **一直在把仓库里真正的仪器文件写掉**
> （409 行 → 1 行，而它自己的断言全绿）。两件事都在
> [`128`](128-the-chain-changed-the-heat-and-never-measured-it.md)。


`scripts/host/zl1-heat-fix-chain-selftest.sh`：**107 检查 / 0 失败**，一次干净的运行归档在
...
| **把 governor 重新挂到证明上**（§4.4.1） | 一次证明不干净的 boot **两半都装不进去**（正是本轮改掉的那个行为） |
| **让 kill 变成无条件的**（§4.4.1 的反方向） | 一个没授权 kill 的 verdict 上 keeper 真的被退休了——这一条要是活着，代价是手机失去地址 |
