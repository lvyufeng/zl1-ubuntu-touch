...
| **发热** | **三个原因都装上了，③的效果量出来了，②跑过三次而三次都没有量到价钱**（2026-09-25；docs 164 / 166 / 167 / 170 / 171 / 172 / 173 / 174） | ① v63 debug keeper 每秒 `systemctl` 一次（约一个核）——退休 unit 每次开机解掉；② 镜像把四个核**全钉在 `performance`**——`install-cpufreq-governor.sh`；③ **SoC 被禁止用自己低功耗阶梯**（每条 cmdline 都带 `lpm_levels.sleep_disabled=1`）——`install-lpm-sleep-fix.sh`，参数读回 `N`。**③ 的价钱量出来了**：`zl1-ladder-temp-ab.sh` 三窗口 A/B/C 在设备上跑过一次，**挡住阶梯让最热的 tsens zone 高 5.5 °C**，控制窗口回到 0.0 °C、而且比第一个窗口还低 0.7–1.3 °C（效应大过这一趟的漂移，方向还相反）。**②的仪器（`zl1-governor-temp-ab.sh`，docs 169）跑过三次**：第一次判词 `CONTAMINATED`（docs 170）——**那一趟没有量到价钱，而这是控制窗口在正常工作**。改完之后**第二次（docs 172）印出了 `COST-MEASURED` 3.5 °C，而那个数是 artifact**：这一趟的手机**整趟掉了 7.7 °C**（A 最热 49.6 → C 41.9），印出来的 3.5 是 21 个 tsens zone 里 **19 个小正数中最大的那一个**，而**最热的两条在每个窗口都在掉**。三个缺陷是同一个：那个"等"**单向**（低 4.2 °C 也算"回来"，10 秒就回来了）、比的是**最热的 zone**（那是会换手的最大值，等于拿一条 zone 的数字减另一条的）、而**窗口 A 本身**读在一条每十秒掉 4.2 °C 的曲线上。**三个都修了（离线，harness 161 → 212 条全绿）**：比较换成 `max_dev`（逐 zone、**绝对值**、印出**符号**和 zone 名），**C 之前**和**A 之前**各一条 hold，手机不停就是 `no-plateau`（退出 1，**唯一一个什么都不写的出口**）。**改完之后已经上过设备（docs 173，第三次运行）**：pre-hold 在真机上跑通（20 秒、最大变化 0.4 °C/10 s），判词是 **`NO RETURN`** 而且**印出了方向**（`4.1 BELOW window A's reading of the SAME zone`），**窗口 C 一次都没读**，四个核被放回并复核——**上一版在同一台手机、同一个形状下会印 `COST-MEASURED 3.5 °C`**。那一趟又找到两条缺陷（都还没修）：hold 的**上界数的是睡眠秒**（每次采样还要 ~23 秒走 `/proc`，所以 `--settle-back 240` 实际花了 **798 秒墙钟**，整趟 1009 秒≈17 分钟，而 `--status` 承诺 12 分钟）；以及 **pre-hold 的判据是一条速率门**（0.5 °C/10 s = **3 °C/分钟**，是它要保护的 0.2 °C 分辨率的 **150 倍**）——它把一台正在以 2.4 °C/分钟移动的手机认证成了"站着不动"。**两个缺陷都已经修了（离线，docs 174），而它们是同一个错误的两半：一个数用一种单位，去比另一把另一种单位的尺。**① 那个"上界"数的是**睡眠秒**：每次采样是 10 秒的仪器运行，而它还要多花约 23 秒走 `/proc`，所以 `--settle-back 240` 花了 **798 秒墙钟**（整趟 1009 秒≈17 分钟，而 `--status` 承诺 12 分钟）——两条 hold 现在都按**时钟**计（`/proc/uptime`；这块板子的 RTC 读 1970，而一个会跳的时钟会让 hold 因为和温度无关的原因结束），并且每个量过的出口都**印出它花了多少**：窗口、hold、以及"它要的睡眠"和"它实际花的时间"之差。② pre-hold 的判据是**一条速率门**（两次相隔 `--poll` 的读数在 `--margin` 以内），而 `0.5 °C / 10 s = 3 °C/分钟` 是它要保护的那个 0.2 °C 分辨率的 **150 倍**；`--margin` 是**位移**——花在整趟运行上，这正是窗口 C 之前那个 wait 一直在测的东西——所以 pre-hold 给一个区间的额度现在是 `--margin × 区间 / RUN_SPAN`，它判的是"刚才看到的移动**外推到整趟**是多少"。默认值下这个额度是 **0.012 °C**，比仪器自己的 0.1 °C 步进还小：**这台手机过不了这条 hold，而这正是 docs 173 的结论变成了可执行的东西**——要改的是**设计**（用 A B A B 交替把漂移在一阶上抵消），不是尺。**两条修法都没有上过设备**；上设备之后变的是**仪器比 docs 173 那一版更早拒绝**（在第一个区间就拒绝，而不是把 pre-hold 的上界跑满），而且仍然什么都不写。 | **"这台机器不发烫了"还没有被说过，也不该说**：那是**三趟、一个负载（0.80 核忙）、环境温度和充电状态都没控制**的读数；脚本自己把限定印在判词里，而**三趟的判词都不是价钱**。**②的价钱仍然没有**：`CONTAMINATED` 没有量到它，`COST-MEASURED 3.5 °C` 也没有（那是 artifact），第三次**直接拒绝**。热时间常数仍然没量过——而 docs 174 之后这条仪器**不再要求"手机站着不动"**：pre-hold 的判据挂在**整趟的位移**上（默认额度 0.012 °C），所以它在真机上应当**在第一个区间就拒绝**这台手机，而**这个预测还没有被量过**。**设计本身现在有了第二个形状（docs 175）**：`--pairs N`（3..30）跑 N 对（fix 窗口、pinned 窗口），**每一对内部的顺序交替**——第 1 对先 fix 后 pinned、第 2 对反过来——读数是每对之差 `B - A` 的**均值**，误差棒是这些差的**散布**，所以会漂移的手机是被**定价**而不是被拒绝，判词还可以是 **`inconclusive`（退出 1）**，那是三窗口设计说不出来的一个答案；**pre-hold 和 wait 在这个设计里被跳过**，因为它们的存在是为了让窗口 A 成为一个**状态**，而配对之差不需要。交替顺序是**承重的**而不是装饰：若每一对顺序相同，pinned 窗口永远比它自己的 fix 窗口**晚一个窗口**，于是每一条差里都带着**同符号**的一个窗口的漂移，均值就是效应**加上**那个漂移（离线：fixture 里真实的 2.0 °C 会读成 3.00，m16 把它放回去）；主 zone 在**第一个窗口**就声明（N 个均值里最大的那个即使毫无效应也会高出两倍标准误，那正是 docs 172 印成价钱的形状）；**balance**（两半各自在时间上的平均位置）由运行**实际记录**的顺序算出（`$TMP/order`），所以取错顺序的运行报告的是**它自己造成的失衡**，并且那个失衡会被它自己量到的漂移折算成 °C。**这个形状也还没有上过设备**（harness 242 → 321 条全绿）；②的价钱仍然没有。**而它现在上过设备了（docs 176，2026-09-26，同一趟启动里六次）**，结果不是设计期待的那一种：同一个设计在同一台手机上印出 **+2.03 / −0.58 / +2.48 / +5.88 / +2.27 / +2.30 °C**，落在**四个不同的主 zone** 上（`zone18 / 12 / 18 / 9`）——因为"主 zone"是 **A.1 里最热的那个传感器**，而**最热的传感器会换手**（docs 172 的老毛病，现在出现在预注册里面）。原因不在设计而在**读数的粒度**：`zl1-thermal.sh` 先把头印出来（头里的 `uptime` 是窗口的**开始**）、`sleep` 满窗口、走一遍 `/proc`，**然后才读 zone**——所以那个数是窗口**末尾的一个瞬间**，而 zone18 在它自己的 48 s 窗口里走过 **41.7 → 45.6 → 43.7**。第 6 次运行时**并行**跑了一个 2 s 的**按名字**采样器（`cat /sys/class/thermal/thermal_zone*/temp`，不靠列号），379 行，给出**窗口均值**下的 **zone18 +0.61 [0.42, 0.80]**、zone12 +2.29、zone9 +1.71；而仪器同一次跑印 +2.30 / ~+6.5 / ~+5。**同一次运行、同一个配对，两种都合法的读法差 1.7 °C，是效应本身的四分之三**（fix 窗口从热里降下来所以末尾偏低、pinned 窗口升上去再平所以末尾偏高——快照法**系统性地把两半推开**）。**而仪器没有读错**：两次按名字核对，它的 zone 读数和原始文件在同一秒一致（含单位表）。90 s 窗口**几乎不动**这个数（第 5 次 zone18 = +2.27），因为噪声在**秒**的量级——**除非把读数改成窗口内的平均**。顺带量到三件事：`performance` 的真身是**镜像的上限** `scaling_max_freq` = **1440000/1440000/1900800/1900800**（`cpuinfo_max_freq` 是 2188800/2342400），4 分钟 60 个采样**只有一个值**、`/sys/class/thermal/` 下**没有 cooling device**，所以它不是温度压下来的；`performance` 那 84 s 里 `cpu0` 的 `cur_freq` **恒为 1440000**（`interactive` 在 307200–1363200 之间动），所以"写入生效"第一次被证到**频率**一级；以及 `thermal_zone12` 在**空载**下 4 秒走 **412→480→406→429→467**，而它正是第 2、3 次的主 zone。**②的价钱仍然没有**，但现在有了一个**区间**（zone18 上 0.6–2.3 °C）和一个**被量出来的偏差**（1.7 °C）。**而这一轮的下一步已经做完了（docs 177，同一天、同一趟启动）**：温度读数改成**窗口内的均值**（`take_window` 每秒走一遍 zone，`thermal_summary` 报均值、样本数、走过的秒数、最宽摆幅），**并且顺手量出同一个文件里的第二个不一致**——窗口里的**分子**是两个 `/proc/stat` 快照之差（一整个窗口），**分母**却是**要求的**秒数；设备上量出来 `--seconds 10` 的两次 `/proc/stat` 相隔 **20.7 s**，所以**这个仓库里每一个 `/s` 都大了 2.07 倍**（zone 遍历 0.30 s + 两趟 `/proc` 遍历各 4.58 s，都在窗口**里面**；`--seconds 10` 端到端 23.8 s）。**修完之后同一个设计又跑了四次**：读数 **+2.65 / +2.20 / +2.73 / +2.10 °C**（主 zone 预注册，`zone12`/`zone9` 各两次），误差棒 **0.10–0.18**（旧读数是 0.38–0.97），**四次都是 `COST-MEASURED`**——而旧读数给的六次是 +2.03 / −0.58 / +2.48 / +5.88 / +2.27 / +2.30。**所以②的价钱现在是一个窄区间（2.1–2.7 °C），不是一个定值**：四次运行**全在同一趟启动里**，负载、环境温度和充电状态都没控制。 |
...
* **不声称"硬件都驱动了"。**表里 14 项：**5 项已证明**、**4 项部分**、**1 项不存在**、
  **3 项只量过离线**、**1 项（发热）三个修复都装上了，③的效果量出来了，②上过设备十三次（三窗口三次 + 交替配对六次 + 均值读数四次）而其中只有最后四次印出了一个能引用的价钱**——六次旧读数给出的是**一个区间和一个被量出来的偏差**（docs 176），四次均值读数给出**2.10–2.73 °C、误差棒 0.10–0.18、四次 `COST-MEASURED`**（docs 177）（2026-09-26 更新；docs 164/166/167/170/172/173/174/175/176/177）。
