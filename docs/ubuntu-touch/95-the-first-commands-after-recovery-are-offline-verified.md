...
而"前几条命令"恰好是两个**没在真机上跑过一次**的脚本：`device/zl1-edl-postmortem.sh`（健康检查第 0 条）和 `device/zl1-boot-address-check.sh`（第 0b 条，它决定第 0d 条能不能做）。两个脚本此前都只用**一次性**的合成测试跑过：`86` §5 记着手工跑的 5 个 case、`88` 记着 4 份日志，**都不可重跑**——而那两次各抓到过真 bug（`86`：一个反引号让脚本真的去执行 `reboot edl`；`94`：按 cmdline 子串匹配把测试用的 shell 自己杀了）。

**这一轮把一次性变成可重跑，重跑之后又抓到三个真 bug**，而且三个都在"回来那一刻要跑的那条链"上。
...
| 怎么保证测的是**真脚本**而不是它的复制品？ | 两个 harness 都用 `sed` 把**真脚本**的设备路径改到一棵假根树里，跑那一份；并且各带一道"改写真的生效了吗"的自检（`sh -n` + `D=`/`NETLOG=` 落地检查）。逻辑一个字都没重写 |
...
| 动设备了吗？ | 没有。两个 harness 都在宿主机上，假根树里的设备状态是编的；没有分区写、没有 boot 镜像写、没有 QDL/firehose、没有重启 |
...
## 3. 两个 harness 各自钉住了什么

**`host/zl1-boot-address-selftest.sh`（6 份日志 / 10 项）** —— 判语授权的是"杀 keeper"（`94`），所以三个结论加开机边界都要钉住。其中两份日志存在的理由：
...
**`host/zl1-edl-postmortem-selftest.sh`（7 种状态 / 34 项）** —— 其中 C 与 C2 是同一个状态的两条到达路径，C2 就是第一次跑就红了的那个。F（pstore 目录和 kmsg 目录都不存在）额外用 `notwant` 断言它**不能说 FOUND**、也**不能把读不到说成没有**——这正是 `86` §7 承诺的性质，也是"post-mortem 是证据还是故事"的分界线。
...
| `scripts/host/zl1-health-check.sh` | 改：第 0b 条的说明里加上"这个判语可以在没有设备的时候先验" |
