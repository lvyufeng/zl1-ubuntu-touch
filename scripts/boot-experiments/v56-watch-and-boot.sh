...
set +e
SER="33e80afe"
IMG="/mnt/data/halium-zl1-candidates/halium-boot-zl1-filtered-dtb-postswitch-debug-v56-minimal-usb-fakebind.img"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUTDIR="/mnt/data/zl1-bb10/v56-minimal-usb-fakebind-boot-${STAMP}"
LOG="${OUTDIR}.log"
...
collect_android_fallback() {
    AOUT="/mnt/data/zl1-bb10/v56-postfallback-adb-$(date -u +%Y%m%dT%H%M%SZ)"
    mkdir -p "$AOUT"
    {
...
log "V56 watcher start target=$SER image=$IMG"
...
for i in $(seq 1 120); do
    if fb_has_target; then
        log "target fastboot detected iteration=$i; fastboot boot V56 only"
    ...
    if adb_has_target; then
        ...
        for j in $(seq 1 35); do
            if fb_has_target; then
                log "target fastboot detected after adb reboot; fastboot boot V56 only"
...
for s in $(seq -w 1 180); do
    ...
    if [ "$android_adb_seen" -ge 3 ] && [ "$reachable_count" -eq 0 ]; then log "normal Android ADB returned without RNDIS; collecting fallback diagnostics"; collect_android_fallback; break; fi
    if [ "$reachable_count" -ge 100 ] && [ "$lost_after_reachable" = 0 ]; then log "V56 remained reachable long window"; break; fi
    if [ "$lost_streak" -ge 15 ]; then log "V56 lost RNDIS after reachable"; break; fi
    sleep 3
done
