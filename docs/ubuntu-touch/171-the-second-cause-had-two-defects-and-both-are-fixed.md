...
**那一次运行做完了，而这台仪器又被改了一次**（[`172`](172-the-second-price-was-an-artifact.md)）：
"等"确实在真机上走通了（10 秒就"回来"了），但**它的判据是单向的**——低 4.2 °C 也算回来——
于是那一趟印出了 `COST-MEASURED 3.5 °C`，而手机**整趟掉了 7.7 °C**。这一页里的 `--margin` 那条
"等读数回到容差以内"当时**只写了往上的一半**，而窗口 A 之前**什么都没有**。
