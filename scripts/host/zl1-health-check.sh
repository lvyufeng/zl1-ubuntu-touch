...
always "      DSPs); the firmware is on the dedicated FAT16 'modem' partition, which /vendor/etc/fstab.qcom"
always "      mounts at /vendor/firmware_mnt with shortname=lower (hence MODEM.MDT for modem.mdt); and the"
always "      UT rootfs has NO /lib/firmware at all. THAT IS A HYPOTHESIS WITH A NAME, not a finding --"
always "      the port may bind-mount the vendor tree, set firmware_class.path, or fetch it some way not"
always "      visible in the images. This probe is what measures which, on the device."
...
always "   79 checks, and it found three defects on the way: a search-path section that concluded 'the"
