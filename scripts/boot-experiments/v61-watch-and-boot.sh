...
set +e
SER="33e80afe"
IMG="/mnt/data/halium-zl1-candidates/halium-boot-zl1-filtered-dtb-postswitch-debug-v61-postinit-monitor.img"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUTDIR="/mnt/data/zl1-bb10/v61-postinit-monitor-boot-${STAMP}"
LOG="${OUTDIR}.log"
...
collect_android_fallback() {
    AOUT="/mnt/data/zl1-bb10/v61-postfallback-adb-$(date -u +%Y%m%dT%H%M%SZ)"
    mkdir -p "$AOUT"
    {
...
collect_v61_monitor_after_loss() {
    MOUT="/mnt/data/zl1-bb10/v61-monitor-collect-$(date -u +%Y%m%dT%H%M%SZ)"
    mkdir -p "$MOUT" 2>/dev/null || true
    log "waiting briefly for target adb/recovery to collect V61 persistent monitor log"
    for k in $(seq 1 90); do
        if adb_has_target; then
            st="$(adb_state)"
            log "target adb visible for V61 monitor collect state=${st:-unknown}"
            {
                echo "timestamp=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
                echo "state=${st:-unknown}"
                echo '--- adb devices ---'; adb devices
                echo '--- mount ---'; adb -s "$SER" shell 'mount 2>/dev/null || cat /proc/mounts 2>/dev/null || true' 2>&1
                echo '--- possible monitor paths ---'; adb -s "$SER" shell 'ls -l /userdata/zl1-v61-monitor.log /data/zl1-v61-monitor.log /sdcard/zl1-v61-monitor.log 2>/dev/null || true' 2>&1
            } > "$MOUT/basic.txt" 2>&1
            adb -s "$SER" exec-out 'cat /userdata/zl1-v61-monitor.log 2>/dev/null || cat /data/zl1-v61-monitor.log 2>/dev/null || cat /sdcard/zl1-v61-monitor.log 2>/dev/null || true' > "$MOUT/zl1-v61-monitor.log" 2>"$MOUT/zl1-v61-monitor.err" || true
            ...
            log "collected V61 monitor diagnostics to $MOUT"
            return 0
        fi
        sleep 2
    done
    log "target adb not visible for V61 monitor collect; no persistent log collected yet outdir=$MOUT"
    return 1
}
log "V61 watcher start target=$SER image=$IMG"
...
for i in $(seq 1 720); do
    if fb_has_target; then
        log "target fastboot detected iteration=$i; fastboot boot V61 only"
    ...
    if adb_has_target; then
        ...
        for j in $(seq 1 45); do
            if fb_has_target; then
                log "target fastboot detected after adb reboot; fastboot boot V61 only"
...
if [ "$booted" != 1 ]; then log "target not seen in adb/fastboot; no boot attempted"; exit 3; fi
...
for s in $(seq -w 1 240); do
    ...
    if [ "$android_adb_seen" -ge 3 ] && [ "$reachable_count" -eq 0 ]; then log "normal Android ADB returned without V61 RNDIS; collecting fallback diagnostics"; collect_android_fallback; break; fi
    if [ "$reachable_count" -ge 120 ] && [ "$lost_after_reachable" = 0 ]; then log "V61 remained reachable long window"; break; fi
    if [ "$lost_streak" -ge 15 ]; then log "V61 lost RNDIS after reachable"; collect_v61_monitor_after_loss || true; break; fi
    sleep 3
done
