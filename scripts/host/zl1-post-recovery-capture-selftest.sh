...
CALLEE_NAMES='EDLPM|BOOTADDR|KEEPER|HEALTH|MODEM|SLEEP|LMH|LEDS|GPS|FP|ORIENT|NOEDL'
...
want '^CALLEE ORIENT$' "$(printf '%s\n' "$O" | sed -n '9p')" "then the orientation survey"
[ "$(order | wc -l)" = 9 ] && ok "exactly nine steps -- the four read-only probes ARE among them and the 05/06 pair are NOT" \
...
[ "$(scpacts | wc -l)" = 7 ] && ok "seven pushes: the seven DEVICE steps, and no more" \
...
for f in 01-edl-postmortem 02-boot-address 03-keeper-status 04-health-check 04b-modem 04c-sleep-throttle 04d-lmh 04e-leds 07-orientation; do
...
[ "$(find "$OD" -name '*-*.txt' | wc -l)" = 10 ] && ok "and the file set does not grow (10 archived outputs, not 20)" \
...
[ "$(order | wc -l)" = 9 ] && ok "leaving nine steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 9 ] && ok "leaving the same nine steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 10 ] && ok "ten steps -- the nine of the default plus the one that writes" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 9 ] && ok "all nine ran (the default set; no 05/06 probe is in it)" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 8 ] && ok "all eight steps still ran" || bad "$(order | wc -l) steps ran"
