...
CALLEE_NAMES='EDLPM|BOOTADDR|KEEPER|HEALTH|MODEM|SLEEP|GPS|FP|ORIENT|NOEDL'
...
want '^CALLEE ORIENT$' "$(printf '%s\n' "$O" | sed -n '7p')" "then the orientation survey"
[ "$(order | wc -l)" = 7 ] && ok "exactly seven steps -- the two read-only probes ARE among them and the 05/06 pair are NOT" \
...
for f in 01-edl-postmortem 02-boot-address 03-keeper-status 04-health-check 04b-modem 04c-sleep-throttle 07-orientation; do
...
[ "$(order | wc -l)" = 7 ] && ok "leaving seven steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 7 ] && ok "leaving the same seven steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 8 ] && ok "eight steps -- the seven of the default plus the one that writes" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 7 ] && ok "all seven ran (the default set; no 05/06 probe is in it)" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 6 ] && ok "all six steps still ran" || bad "$(order | wc -l) steps ran"
