...
always "      offline: scripts/host/zl1-loc-fp-selftest.sh, 181 checks. It has found four real defects in"
...
always "      Every existence test it makes now reads through /proc/<hal-pid>/root/... (docs 116). Before,"
always "      they were 'nsenter -t <pid> -m -- test -e <path>', and nsenter resolves the COMMAND in the target's"
always "      mount table -- where the container has no /usr/bin, so test(1) was never found, nsenter exited"
always "      127, and every one of those tests answered 'missing' regardless of what was there. Nine such"
always "      errors sit in the archived 2026-09-24 probe output; section 2's 'no variant match' was a"
always "      conclusion the probe had not earned. /proc/<pid>/root/<path> needs no exec."
