...
* **不声称"硬件都驱动了"。**表里 14 项：**5 项已证明**、**4 项部分**、**1 项不存在**、
  **3 项只量过离线**、**1 项（发热）三个修复都装上了，③的效果量出来了，②上过设备九次（三窗口三次 + 交替配对六次）而每次印出来的都不是一个可引用的价钱**——但交替配对六次给出了**一个区间和一个被量出来的偏差**（docs 176）（2026-09-26 更新；docs 164/166/167/170/172/173/174/175/176）。
