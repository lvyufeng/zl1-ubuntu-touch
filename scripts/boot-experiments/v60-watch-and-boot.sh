...
set +e
SER="33e80afe"
IMG="/mnt/data/halium-zl1-candidates/halium-boot-zl1-filtered-dtb-postswitch-debug-v60-repro-fakebind.img"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUTDIR="/mnt/data/zl1-bb10/v60-repro-fakebind-boot-${STAMP}"
LOG="${OUTDIR}.log"
...
collect_android_fallback() {
    AOUT="/mnt/data/zl1-bb10/v60-postfallback-adb-$(date -u +%Y%m%dT%H%M%SZ)"
    mkdir -p "$AOUT"
    {
...
log "V60 watcher start target=$SER image=$IMG"
...
for i in $(seq 1 720); do
    if fb_has_target; then
        log "target fastboot detected iteration=$i; fastboot boot V60 only"
    ...
    if adb_has_target; then
        ...
        for j in $(seq 1 45); do
            if fb_has_target; then
                log "target fastboot detected after adb reboot; fastboot boot V60 only"
...
for s in $(seq -w 1 240); do
    ...
    if [ "$android_adb_seen" -ge 3 ] && [ "$reachable_count" -eq 0 ]; then log "normal Android ADB returned without V60 RNDIS; collecting fallback diagnostics"; collect_android_fallback; break; fi
    if [ "$reachable_count" -ge 120 ] && [ "$lost_after_reachable" = 0 ]; then log "V60 remained reachable long window"; break; fi
    if [ "$lost_streak" -ge 15 ]; then log "V60 lost RNDIS after reachable"; break; fi
    sleep 3
done
