...
| `install-kmsg-drain.sh` | `--install` / `--remove` / `--status` / `--read [LINES]`. Installs a `DefaultDependencies=no` unit (script and log on `/userdata`, unit on the `/etc/systemd/system` writable-path) that snapshots the whole kernel ring buffer to `/userdata/zl1-kmsg/boot.log` at ~t=10 s — `dmesg` uses `SYSLOG_ACTION_READ_ALL`, so the snapshot still holds the boot-time driver messages — then follows `/dev/kmsg` into `kmsg.log` with 8 MB rotation. Needed because the `tx_complete` WARN storm from our own uether patch wraps the ring every ~18 s and `journalctl -k` returns 1 line, which is what made the Wi-Fi work blind; `--read` greps both files for `cnss|wlan|wcnss|qca6174|ar6320|qcacld`. **Not yet run on the device** — it was written while the device was in EDL, see [`../docs/ubuntu-touch/49-two-cores-that-are-not-tls-wifi-stuck-at-wcnss-and-a-trip-into-edl.md`](../docs/ubuntu-touch/49-two-cores-that-are-not-tls-wifi-stuck-at-wcnss-and-a-trip-into-edl.md). |
