...
| 问题 | 答案 |
|---|---|
| 发现/改了什么？ | 指纹探针的**每一条存在性测试都从来没有得到过答案**：`nsenter -t PID -m -- test -e PATH` 里的 `test` 是在**容器的挂载表里**被 exec 的，而那个镜像里**没有 `test` 这个 applet**（mksh 把 `test` 做成内建，toybox 的那个没有装），于是 `nsenter` 以 127 退出 —— shell 把 exec 失败读成**假**，所以"测不出来"被报成了"不存在" |
...
| 为什么这个错值得单开一篇？ | 这是 `103`/`109` 那条规则**同一形状的第二层**：一个仪器**不能报数**，却印出了一个确定的答案。而且它不只影响阅读 —— 唯一那个会写的模式（`--create-store-dir`）也用同一套测试，所以守卫**从来没有生效过**，创建分支每次都被走到（`mkdir` 在容器里是有的，见 §2 的修正） |
...
里被解析的；而在那个表里，`test` **不在任何 PATH 目录中**：它是 `nsenter` 要 exec 的一个真程序，而这个
Android 镜像的 applet 集合里没有它（§2 的修正；同一集合里 `ls`/`mkdir` 都有）。所以：
...
`test -e`/`mkdir`/`chown`/`chmod`/`ls -ldn`）。

**⚠️ 修正（docs 117）**：本文最初在这里写的是"`mkdir` 同样以 127 失败，所以这个探针从来没有在设备上
创建过任何东西"。**那是错的，本节的下半段已按 117 重写。** 127 只发生在**容器没有那个 applet** 的命令
上，而这是**逐命令**的：`test` 和 `[` 确实不在容器的 applet 集合里（mksh 把 `test` 做成内建，toybox 的
那个 applet 没有装进这个镜像），但 `mkdir`/`chown`/`chmod`/`ls`/`ps`/`rmdir`/`grep`/`awk`/`sed`/`tr` 都在。
所以真相是反过来的：那个 `[ -e ]` 守卫每次都答"不存在"，于是创建分支**每次都被走到**，而
`mkdir -p`/`chown`/`chmod` **会成功**。唯一那个会写的模式不是没有写，而是**它的守卫从来没有生效过**
（"已经存在"那条分支是死代码），而它的输出会看起来完全正确。详见
[`117`](117-the-command-that-could-not-run-is-not-a-zero.md) §2，那里的核对是拿设备自己的
`system.img` 做的。
...
`/vendor`；第二稿把它改成 `nsenter -m -- test`，交给一个没有这个 applet 的挂载表，于是"不存在"的
...
**而这条推理链在 docs 117 被推翻了。** 本文原来说：那个会写的模式因为同一个 127 而什么都没写，所以
探针"实际上一直是只读的"，于是两次 EDL 都不可能是它写出来的。**这句话是错的** —— `mkdir` 在容器里是
存在的（§2 的修正），所以它**会**写。

那么"是否留下过东西"就变成一个**可以读**的问题，而不是可以推理的问题：2026-09-24 那次的读数里，
`/data/system/users/0/fpdata`、`/data/vendor_de/0/fpdata`、`/data/vendor/biometrics` 三条**全是
MISSING**，而那次读是走 `/proc/<hal-pid>/root/...`（也就是**会答话**的那个机制）拿到的。`/data` 跨开机
存在，所以这是证据：**没有找到任何一次早先的 `--create-store-dir` 留下的目录**。它不能证明没跑过，只能
证明跑过也不会在这三条路径上留下东西——而这三条正是它可能会碰的全部。
