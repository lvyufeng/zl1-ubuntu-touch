...
**【更正，见 [`94`](94-retiring-the-v63-debug-keeper-is-a-kill-not-a-unit-edit.md)：上面那个"改 unit 的 `ExecStart`"**不可能生效**。**v63 的引导钩子 `zl1-postswitch-debug-init` **每次开机都重写** `/usr/local/sbin/zl1-debug-net.sh`（第 313 行）和 `/etc/systemd/system/zl1-debug-net.service`（第 632 行，连两个 `*.target.wants` 符号链接一起，648-649），所以改哪个下一个开机就没了。看起来能活下来的 drop-in（`/etc/systemd/system/zl1-debug-net.service.d/`，钩子不碰它）**也救不了**：钩子第 1587-1588 行在 systemd 之前就把 keeper 直接后台启动了，keeper 的互斥是原子的 `mkdir /run/zl1-debug-net.lock`，ramdisk 那个实例先拿到锁，systemd 那个实例打一行 `keeper duplicate exit v63` 后 `exit 0`，所以 unit 整轮都是 `inactive (dead)` —— **`ExecStart` 覆盖的是那个本来就会放弃的实例**。剩下唯一不用动引导镜像的杠杆是 keeper 自己的形状：纯 shell、由 ramdisk 启动、没有任何东西会重启它，所以**开机后杀掉它，到下次重启为止不会回来**（每次开机要再来一次）。`scripts/install-retire-debug-keeper.sh` 就是这件事：先 `mask --runtime` 它的 unit，等地址出现（等不到就**拒绝动手**），再 TERM/KILL 并观察 30 秒，把"杀不掉"和"被重启"分开报告。**】
