...
always "   1b. the orientation value -- the \"it keeps going landscape\" bug (docs 91), and it needs"
always "       one hand: the phone flat, screen up"
always "                                                scp scripts/device/zl1-orientation-axes.sh root@$IP:/tmp/ && \\"
always "                                                ssh root@$IP 'sh /tmp/zl1-orientation-axes.sh --seconds 30 --flat-up'"
always "      (read-only, changes nothing. It compares the classifier's 5/6 against the sign of the"
always "       accelerometer's z -- the value and its own input, docs 91 -- and answers INVERTED /"
always "       AGREES / AMBIGUOUS / no flat sample. INVERTED licenses the one-line matrix trial and"
always "       --explain prints the reversible way to run it; nothing is changed by the probe)"
