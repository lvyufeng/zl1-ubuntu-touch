...
edl)
  ...
  always "   The SoC is in the emergency download loader. There is no network, no SSH, no shell:"
  always "   the only thing this port does is wait for a firehose downloader, which this project"
  always "   does not use (\"在 EDL 中不要使用 QFIL 类工具\" -- and nothing here has ever written a"
  always "   partition, so there is nothing to download anyway)."
  always ""
  ...
  always "   Then it should re-appear as the RNDIS gadget (port may change); re-run this script."
  always "   After it boots, the self-check below runs by itself; the two things that prove it is the"
  always "   right device are the device-tree model and that \`adb devices\` has no $SERIAL_PREFIX."
  always ""
  always "   Do NOT: run QFIL/qdl/firehose, unbind cnss/cnss_pci (docs 49: that is what caused one of"
  always "   these), or flash anything. There is no software exit from EDL on this device."
  always ""
...
always ""
always "== the four measurements that are owed, and what each answers"
