...
always "   evidence that dies with this boot matters. Behaviour pre-verified offline:"
always "   scripts/host/zl1-post-recovery-capture-selftest.sh, 94 checks, and five mutations each"
...
always "       zl1-boot-address-selftest.sh runs the real script against six synthetic netwatch logs,"
always "       including two with two boots in the log, where reading the wrong section inverts it)"
...
always "       (78 checks, transport stubs are the device; 23 of them go red against the old script)."
always "       doc 104)"
...
always "      Offline and device-free: scripts/host/zl1-gps-selftest.sh, 69 checks (its section 9 is"
...
always "   2b. and then the request that has never been made at all (docs 93): the hardware entry"
always "       points u_hardware_gps_new/u_hardware_gps_start are only reached from a client's"
always "       StartPositionUpdates, and a default-closed trust-store gate stands in front of it"
...
always "      offline: scripts/host/zl1-loc-fp-selftest.sh, 179 checks. It has found three real defects in"
...
always "      130 checks -- and four mutations of the installer (no read-back, no refusal gate, a host"
