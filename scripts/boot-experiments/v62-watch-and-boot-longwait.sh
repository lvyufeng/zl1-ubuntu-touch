...
set +e
SER="33e80afe"
IMG="/mnt/data/halium-zl1-candidates/halium-boot-zl1-filtered-dtb-postswitch-debug-v62-single-keeper.img"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUTDIR="/mnt/data/zl1-bb10/v62-postinit-monitor-boot-${STAMP}"
LOG="${OUTDIR}.log"
...
collect_android_fallback() {
    AOUT="/mnt/data/zl1-bb10/v62-postfallback-adb-$(date -u +%Y%m%dT%H%M%SZ)"
    mkdir -p "$AOUT"
    {
...
collect_v62_monitor_after_loss() {
    MOUT="/mnt/data/zl1-bb10/v62-monitor-collect-$(date -u +%Y%m%dT%H%M%SZ)"
    ...
    for k in $(seq 1 90); do
        if adb_has_target; then
            ...
            log "target adb visible for V62 monitor collect state=${st:-unknown}"
            {
                ...
                echo '--- possible monitor paths ---'; adb -s "$SER" shell 'ls -l /userdata/zl1-v62-monitor.log /data/zl1-v62-monitor.log /sdcard/zl1-v62-monitor.log 2>/dev/null || true' 2>&1
            } > "$MOUT/basic.txt" 2>&1
            adb -s "$SER" exec-out 'cat /userdata/zl1-v62-monitor.log 2>/dev/null || cat /data/zl1-v62-monitor.log 2>/dev/null || cat /sdcard/zl1-v62-monitor.log 2>/dev/null || true' > "$MOUT/zl1-v62-monitor.log" 2>"$MOUT/zl1-v62-monitor.err" || true
...
for s in $(seq -w 1 240); do
    ...
    if [ "$lost_streak" -ge 15 ]; then log "V62 lost RNDIS after reachable"; collect_v62_monitor_after_loss || true; break; fi
    sleep 3
done
