...
| **发热** | **三个原因都装上了，③的效果量出来了，②的机器跑过一次但判词是"我不知道"**（2026-09-25；docs 164 / 166 / 167 / 170） | ① v63 debug keeper 每秒 `systemctl` 一次（约一个核）——退休 unit 每次开机解掉；② 镜像把四个核**全钉在 `performance`**——`install-cpufreq-governor.sh`；③ **SoC 被禁止用自己低功耗阶梯**（每条 cmdline 都带 `lpm_levels.sleep_disabled=1`）——`install-lpm-sleep-fix.sh`，参数读回 `N`。**③ 的价钱量出来了**：`zl1-ladder-temp-ab.sh` 三窗口 A/B/C 在设备上跑过一次，**挡住阶梯让最热的 tsens zone 高 5.5 °C**，控制窗口回到 0.0 °C、而且比第一个窗口还低 0.7–1.3 °C（效应大过这一趟的漂移，方向还相反）。**②的仪器（`zl1-governor-temp-ab.sh`，docs 169）跑过一次，判词 `CONTAMINATED`**（docs 170）：最热 zone A 40.9 → B 47.7 → **C 48.0**——升温没跟着状态回来，所以那一趟**没有量到②的价钱**，而这是控制窗口在正常工作。那一趟同时发现仪器里一句指向已删目录的话（丢了 26 行 zone 读数）和设计里**没有命名过的第二个盲区**：控制窗口只能抓住在 C 之前**消失**的漂移。 | **"这台机器不发烫了"还没有被说过，也不该说**：那是**一趟、一个负载（0.80 核忙）、环境温度和充电状态都没控制**的读数；脚本自己把限定印在判词里。**②的价钱仍然没有**——`CONTAMINATED` 既没有量到它、也没有反驳它。热时间常数也没量过（它是上面那个盲区的修法的前提）。 |
