...
CALLEE_NAMES='EDLPM|BOOTADDR|KEEPER|HEALTH|MODEM|SLEEP|LMH|GPS|FP|ORIENT|NOEDL'
...
want '^CALLEE ORIENT$' "$(printf '%s\n' "$O" | sed -n '8p')" "then the orientation survey"
[ "$(order | wc -l)" = 8 ] && ok "exactly eight steps -- the three read-only probes ARE among them and the 05/06 pair are NOT" \
...
[ "$(scpacts | wc -l)" = 6 ] && ok "six pushes: the six DEVICE steps, and no more" \
...
for f in 01-edl-postmortem 02-boot-address 03-keeper-status 04-health-check 04b-modem 04c-sleep-throttle 04d-lmh 07-orientation; do
...
[ "$(find "$OD" -name '*-*.txt' | wc -l)" = 9 ] && ok "and the file set does not grow (9 archived outputs, not 18)" \
...
[ "$(order | wc -l)" = 8 ] && ok "leaving eight steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 8 ] && ok "leaving the same eight steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 9 ] && ok "nine steps -- the eight of the default plus the one that writes" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 8 ] && ok "all eight ran (the default set; no 05/06 probe is in it)" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 7 ] && ok "all seven steps still ran" || bad "$(order | wc -l) steps ran"
