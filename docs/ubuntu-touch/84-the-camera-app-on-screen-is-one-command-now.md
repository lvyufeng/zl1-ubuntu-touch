...
## 8. 更正（2026-09-23，见 [`104`](104-the-camera-app-instrument-measured-in-the-wrong-unit.md)）

这一篇交付的那个执行器**当时带着四个缺陷**，其中两个会直接改变从这里读到的东西。给它写离线 harness
（`scripts/host/zl1-camera-app-test-selftest.sh`）时才发现的：

* **它打印的速率是它自己单位的 100 倍。** 上面 §5 说的 "band 20–50/s" 是对的（`68` §5，HZ=100），但
  代码算的是 `(b-a) * 100 / secs`——所以屏幕上出现的 `100`/`2000` 和头部引用的 `1.2`/`20–50` 差两个
  数量级。更要紧的是判语的**绝对门槛 `B >= 8`**（写下来时意思是"8 ticks/s"）实际是 **0.08/s**：除了
  合成器完全停住，它不可能失败。真正的判据只有比值那一半。现在窗口函数只带回 jiffies 和秒，速率在本地
  按文档单位算；而因为速率变成小数，比值判断也整块搬进了 awk（`[ -ge ]` 吃不了小数：
  `bash: 1.0: syntax error: invalid arithmetic operator`）。
* **判语没有读 app 自己的状态。** §4 的第 3 步会打印 `launched pid ...`，窗口 B 之后也会打印
  `alive pid=...` 或 `NOT RUNNING (...)`——**判语一个字都没用**。于是启动器失败（uid、会话总线、EGL、
  二进制缺失）会被报成"app 没有被合成"。现在判语先看 app：**从没起来** / **窗口 B 结束前就死了** /
  两个窗口有一个读不出来 → **NO VERDICT**（老代码在这最后一种情况下**一行都不打**，看起来像"判语恰好
  是空的"）。
* 证据表只数了 `app.err` 还管它叫"app 自己的证据"——而 `zl1-camapp-launch.py` 和 Qt 的消息处理器都写
  stderr。现在两个流都数，列名写明来自哪个文件。
* `--run-seconds` 在头部被写成"必须大于 2 × `--seconds`"，代码里没有任何一行检查它（真正的要求是
  `--seconds + 6 + 2`）。低于它，app 会在窗口 B 中间被杀，判语转而怪 app——所以现在是拒绝（exit 2）。

本篇其余内容（协议本身、为什么截图不算活性、要恢复显示器状态）**没有变**，也被 harness 逐条钉住了。
`scripts/host/zl1-camera-app-test.sh` 到今天为止仍然**没有在设备上跑过**。
