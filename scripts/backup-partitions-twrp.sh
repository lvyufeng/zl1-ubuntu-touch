...
stream_partition() {
  ...
  # Stream exact partition bytes to the host. Host-side truncation to blockdev's
  # reported size prevents oversized/corrupt backups if the transport appends
  # bytes after a block read. This still only reads from the device and never
  # writes device block nodes.
