#!/bin/bash
# V66 Production Keeper Watcher
# Based on V64 watcher — monitors RNDIS + SSH stability

set -uo pipefail

TARGET_SERIAL="33e80afe"
BOOT_IMAGE="/mnt/data/halium-zl1-candidates/halium-boot-zl1-v66-guardian.img"
TIMESTAMP=$(date -u +%Y%m%dT%H%M%SZ)
LOGFILE="/mnt/data/zl1-bb10/v66-guardian-boot-${TIMESTAMP}.log"
OUTDIR="/mnt/data/zl1-bb10/v66-guardian-boot-${TIMESTAMP}"
...
log "V66 watcher start target=$TARGET_SERIAL image=$BOOT_IMAGE"
...
log "target fastboot detected; fastboot boot V66"
...
while true; do
    ...
    if [ $reachable_count -ge 100 ]; then
        log "SUCCESS: V66 RNDIS stable for 100+ samples"
        ...
        mkdir -p "$OUTDIR"
        echo "V66 SUCCESS: stable RNDIS" > "$OUTDIR/result.txt"
        exit 0
    fi

    if [ $lost_streak -ge 15 ]; then
        log "FAILURE: V66 RNDIS lost after $reachable_count reachable samples"
        log "FINAL reachable_count=$reachable_count max_stable=$max_stable ssh_ok=$ssh_ok lost_streak=$lost_streak"
        log "log=$LOGFILE outdir=$OUTDIR"
        mkdir -p "$OUTDIR"
        echo "V66 FAILED: lost after $reachable_count" > "$OUTDIR/result.txt"
        exit 1
    fi
