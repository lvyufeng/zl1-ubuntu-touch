...
CALLEE_NAMES='EDLPM|BOOTADDR|KEEPER|HEALTH|MODEM|SLEEP|LMH|LEDS|VIBR|VIDEO|SDCARD|USBPD|HDMI|WFD|NFC|FM|GPS|FP|ORIENT|NOEDL'
...
want '^CALLEE ORIENT$' "$(printf '%s\n' "$O" | sed -n '17p')" "then the orientation survey"
[ "$(order | wc -l)" = 17 ] && ok "exactly seventeen steps -- the twelve read-only probes ARE among them and the 05/06 pair are NOT" \
...
[ "$(scpacts | wc -l)" = 15 ] && ok "fifteen pushes: the fifteen DEVICE steps, and no more" \
...
for f in 01-edl-postmortem 02-boot-address 03-keeper-status 04-health-check 04b-modem 04c-sleep-throttle 04d-lmh 04e-leds 04f-vibrator 04g-video 04h-sdcard 04i-usbpd 04j-hdmi 04k-wfd 04l-nfc 04m-fm 07-orientation; do
...
[ "$(find "$OD" -name '*-*.txt' | wc -l)" = 18 ] && ok "and the file set does not grow (18 archived outputs, not 36)" \
...
[ "$(order | wc -l)" = 17 ] && ok "leaving seventeen steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 17 ] && ok "leaving the same seventeen steps" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 18 ] && ok "eighteen steps -- the seventeen of the default plus the one that writes" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 17 ] && ok "all seventeen ran (the default set; no 05/06 probe is in it)" || bad "$(order | wc -l) steps ran"
...
[ "$(order | wc -l)" = 16 ] && ok "all sixteen steps still ran (the default set minus the orientation survey)" || bad "$(order | wc -l) steps ran"
