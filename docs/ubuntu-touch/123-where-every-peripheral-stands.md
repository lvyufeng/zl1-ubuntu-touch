...
| **发热** | **三个原因都装上了，③的效果量出来了，②的机器跑过一次但判词是"我不知道"**（2026-09-25；docs 164 / 166 / 167 / 170 / 171） | ① v63 debug keeper 每秒 `systemctl` 一次（约一个核）——退休 unit 每次开机解掉；② 镜像把四个核**全钉在 `performance`**——`install-cpufreq-governor.sh`；③ **SoC 被禁止用自己低功耗阶梯**（每条 cmdline 都带 `lpm_levels.sleep_disabled=1`）——`install-lpm-sleep-fix.sh`，参数读回 `N`。**③ 的价钱量出来了**：`zl1-ladder-temp-ab.sh` 三窗口 A/B/C 在设备上跑过一次，**挡住阶梯让最热的 tsens zone 高 5.5 °C**，控制窗口回到 0.0 °C、而且比第一个窗口还低 0.7–1.3 °C（效应大过这一趟的漂移，方向还相反）。**②的仪器（`zl1-governor-temp-ab.sh`，docs 169）跑过一次，判词 `CONTAMINATED`**（docs 170）：**B-A 最大的那个** zone（不是最热的那个）A 40.9 → B 47.7 → **C 48.0**——升温没跟着状态回来，所以那一趟**没有量到②的价钱**，而这是控制窗口在正常工作。那一趟同时发现仪器里一句指向**没被清理过的**目录的话（288 个草稿目录留在笔记本 `/tmp` 里）和设计里**没有命名过的第二个盲区**：控制窗口只能抓住在 C 之前**消失**的漂移。**两个缺陷都修了（docs 171）**：整张表印出来、运行说出草稿目录在哪、一个 EXIT trap 同时管恢复和清理；窗口 C 之前**等读数回到窗口 A 的容差以内**，没回来就是判词 `no-return`（不印价钱、不读 C）。**改完之后还没再上过设备。** | **"这台机器不发烫了"还没有被说过，也不该说**：那是**一趟、一个负载（0.80 核忙）、环境温度和充电状态都没控制**的读数；脚本自己把限定印在判词里。**②的价钱仍然没有**——`CONTAMINATED` 既没有量到它、也没有反驳它。热时间常数也没量过（它是上面那个盲区的修法的前提）。 |
...
* **不声称"硬件都驱动了"。**表里 14 项：**5 项已证明**、**4 项部分**、**1 项不存在**、
  **3 项只量过离线**、**1 项（发热）三个修复都装上了，③的效果量出来了，②跑过一次而判词是 `CONTAMINATED`**（2026-09-25 更新；docs 164/166/167/170）。
